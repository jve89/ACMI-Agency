...
import type { Lead, LeadMeta } from "../types/leads";
...
function validateLead(input: any): { ok: true; lead: Lead } | { ok: false; error: string } {
  ...
  if (lead.website) return { ok: false, error: "Spam detected" }; // honeypot tripped
...
function subjectFor(lead: Lead, meta: LeadMeta): string {
  // [ACMI LEAD #yyyyMMddHHmm-XXXXXX] Airline ORG→DST YYYY-MM-DD
...
import { sendEmailViaSendGrid, sendSlackNotification } from "../services/mail";
